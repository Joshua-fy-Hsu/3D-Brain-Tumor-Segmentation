"""Slide 17 — Qualitative success: same rendering as conclusion_hero but for hybrid (AURA).

Output: docs/report_figures/qualitative_success.png
"""
import sys
...
CASES = [
    ("BraTS2021_01418", "Patient BraTS2021_01418  |  Mean Dice 0.92"),
    ("BraTS2021_01593", "Patient BraTS2021_01593  |  Mean Dice 0.99"),
]
...
def render_case(fig, axes_row, img, gt, pred, case_label, show_col_titles=True):
    ...
    # case label centred above the row, placed above the middle axes
    axes_row[1].annotate(
        case_label,
...
def main():
    ...
    for row, (img, gt, pred, label) in enumerate(results):
        render_case(fig, axes[row], img, gt, pred, label, show_col_titles=(row == 0))
    ...
    fig.subplots_adjust(left=0.02, right=0.98, top=0.94, bottom=0.06, wspace=0.04, hspace=0.38)
    out = OUT / "qualitative_success.png"
